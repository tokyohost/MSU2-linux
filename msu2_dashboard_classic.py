...
logger = logging.getLogger("MSU2-经典仪表盘")
...
class SystemMonitor:
    ...
    def _log_system_data(data):
        """记录后台线程刚发布的经典仪表盘数据快照。"""
        logger.info(
            "异步数据更新 | CPU=%s%s | 内存=%s%s %s | 磁盘=%s%s %s | 网络=%s",
    ...
    def draw_grid_layout(self, draw):
        """绘制网格布局（2x2）"""
        layouts = [
            {'key': 'cpu', 'position': (5, 5)},      # 左上 (CPU)
            {'key': 'memory', 'position': (5, 25)},   # 左下（内存）
            {'key': 'disk', 'position': (5, 45)},     # 左下（磁盘）
            {'key': 'network', 'position': (5, 65)}   # 最下方（网络）
        ]
